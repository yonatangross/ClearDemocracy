...
namespace Politics.Api.Models.Input
{
    ...
    public class GovernmentInputValidator : AbstractValidator<GovernmentInput>
    {
        ...
        {
            RuleFor(government => government.GovName).NotEmpty().WithMessage("Government name is required.");
            RuleFor(government => government.GovStartDate).NotEmpty().WithMessage("Government start date is required.");
